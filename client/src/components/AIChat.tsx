          ...
          >
            {messages.length === 0 && (
              <div className="h-full flex flex-col items-center justify-center text-center p-6 space-y-6">
                <div className="w-16 h-16 rounded-2xl bg-blue-500/10 flex items-center justify-center text-blue-400/50">
                  <MessageSquare size={32} />
                </div>
                <div>
                  <h4 className="text-white font-medium mb-1">How can I help?</h4>
                  <p className="text-sm text-zinc-400">Ask me anything about the ongoing discussion, or try a quick prompt.</p>
                </div>
                <div className="w-full space-y-2">
                  {QUICK_PROMPTS.map((prompt) => (
                    <button
                      ...
                      className="w-full p-3 rounded-xl bg-white/5 border border-white/10 hover:bg-white/10 hover:border-white/20 transition-all text-left text-xs text-white flex items-center gap-3 group"
                    >
          ...
          <div className="p-4 border-t border-white/10 bg-white/5">
            <div className="relative">
              <input
                ...
                className="w-full bg-white/5 border border-white/10 rounded-xl py-3 pl-4 pr-12 text-sm text-white placeholder-zinc-500 focus:outline-none focus:border-blue-500 transition-colors"
              />
              <button
                onClick={() => handleSend()}
                disabled={!input.trim()}
