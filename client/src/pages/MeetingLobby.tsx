...
export default function MeetingLobby({ meetingCode }: { meetingCode: string }) {
  ...
  return (
    <div className="min-h-screen bg-[#030507] flex items-center justify-center p-6 sm:p-12 font-sans selection:bg-blue-500/30">
      <div className="max-w-6xl w-full grid grid-cols-1 lg:grid-cols-2 gap-12 items-center">
        ...
        <div className="space-y-6">
          <div className="relative aspect-video rounded-[40px] bg-white/5 border border-white/10 overflow-hidden shadow-2xl group transition-all duration-500 hover:border-blue-500/30">
             ...
             <div className="absolute bottom-6 left-1/2 -translate-x-1/2 flex items-center gap-4 z-20">
                <button 
                  onClick={toggleMic}
                  className={`p-4 rounded-2xl transition-all shadow-xl active:scale-90 ${isMicOn ? 'bg-white/10 hover:bg-white/20 text-white border border-white/10' : 'bg-red-500 text-white shadow-red-500/20'}`}
                >
                  {isMicOn ? <Mic size={24} /> : <MicOff size={24} />}
                </button>
                <button 
                  onClick={toggleVideo}
                  className={`p-4 rounded-2xl transition-all shadow-xl active:scale-90 ${isVideoOn ? 'bg-white/10 hover:bg-white/20 text-white border border-white/10' : 'bg-red-500 text-white shadow-red-500/20'}`}
                >
                   {isVideoOn ? <Camera size={24} /> : <VideoOff size={24} />}
                </button>
                <button 
                  onClick={() => setShowSettings(true)}
                  className="p-4 rounded-2xl bg-white/10 hover:bg-white/20 text-white border border-white/10 transition-all shadow-xl active:scale-90"
                >
                  <Settings size={24} />
                </button>
             </div>
             ...
             {/* Visualizer overlay */}
             {isMicOn && (
               <div className="absolute top-6 right-6 flex items-end gap-1 h-8 px-3 py-2 bg-black/40 rounded-xl border border-white/10 blur-0">
                  {[1,2,3,4].map(i => (
                    <div key={i} className="w-1 bg-blue-400 rounded-full animate-audio-bar" style={{ animationDelay: `${i * 0.1}s` }} />
                  ))}
               </div>
             )}
          </div>
        ...
        <div className="bg-white/2 border border-white/5 rounded-[48px] p-10 lg:p-14 shadow-2xl backdrop-blur-3xl relative overflow-hidden">
          ...
          {error ? (
            ...
            <div className="relative z-10">
              <h2 className="text-sm font-black uppercase tracking-[0.3em] text-blue-500 mb-4">Joining Meeting</h2>
              ...
              <div className="flex items-center gap-3 mb-10 pb-10 border-b border-white/5">
                ...
                <div>
                  <p className="text-white/60 text-xs font-bold uppercase tracking-widest">Hosted by</p>
                  <p className="text-white font-bold">{meeting?.host.name || 'Admin User'}</p>
                </div>
              </div>

              <div className="space-y-6">
                <div className="bg-white/5 border border-white/5 rounded-3xl p-6 transition-all hover:bg-white/8">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  <button 
                    onClick={handleJoin}
                    className="group relative bg-white text-black font-black text-sm py-5 rounded-[22px] shadow-2xl overflow-hidden active:scale-95 transition-all"
                  >
                  ...
                  <a 
                    href="#/dashboard"
                    className="flex items-center justify-center py-5 border border-white/10 rounded-[22px] text-sm font-bold text-white/60 hover:text-white hover:bg-white/5 transition-all"
                  >
                    Cancel
                  </a>
                </div>
      ...
      {showSettings && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-black/80 backdrop-blur-md animate-fade-in">
           <div className="bg-[#0a0f1d] border border-white/10 rounded-[40px] w-full max-w-2xl overflow-hidden shadow-[0_32px_128px_rgba(0,0,0,0.8)] relative">
              {/* Header */}
              <div className="p-8 border-b border-white/5 flex items-center justify-between">
                 <div>
                    <h2 className="text-2xl font-black tracking-tight">Meeting Settings</h2>
                    <p className="text-white/40 text-sm font-medium">Configure your hardware for the session</p>
                 </div>
                 <button 
                    ...
                    <div className="grid gap-2">
                       {devices.filter(d => d.kind === 'videoinput').map(device => (
                          <button 
                            ...
                            className={`flex items-center justify-between p-4 rounded-2xl border transition-all ${selectedVideo === device.deviceId ? 'bg-blue-600/10 border-blue-500/50 text-white' : 'bg-white/2 border-white/5 text-white/40 hover:bg-white/5'}`}
                          >
                             <span className="text-sm font-bold truncate max-w-[400px]">{device.label || `Camera ${device.deviceId.slice(0, 5)}`}</span>
                             {selectedVideo === device.deviceId && <Check size={18} className="text-blue-400" />}
                          </button>
                       ))}
                    </div>
                 </div>
                    ...
                    <div className="grid gap-2">
                       {devices.filter(d => d.kind === 'audioinput').map(device => (
                          <button 
                            ...
                            className={`flex items-center justify-between p-4 rounded-2xl border transition-all ${selectedAudio === device.deviceId ? 'bg-violet-600/10 border-violet-500/50 text-white' : 'bg-white/2 border-white/5 text-white/40 hover:bg-white/5'}`}
                          >
                             <span className="text-sm font-bold truncate max-w-[400px]">{device.label || `Microphone ${device.deviceId.slice(0, 5)}`}</span>
                             {selectedAudio === device.deviceId && <Check size={18} className="text-violet-400" />}
                          </button>
                       ))}
                    </div>
                 </div>
              ...
              <div className="p-8 bg-white/[0.02] border-t border-white/5 flex justify-end">
                 <button 
                   onClick={() => setShowSettings(false)}
                   className="px-8 py-4 bg-white text-black font-black uppercase tracking-widest text-xs rounded-2xl hover:bg-blue-50 transition-all active:scale-95 shadow-xl"
                 >
                   Done
                 </button>
              </div>
